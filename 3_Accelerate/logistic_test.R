...
demo_data = dbGetQuery(con, "Select death_inhosp, age, first_sapsii, gender, lda_50_12 
    ...
    ##### train data #####
    train = cbind(as.factor(train_data$death_inhosp),train_data[,2:3],as.factor(train_data$gender))
    ...
    ##### test data #####
    test = cbind(as.factor(test_data$death_inhosp),test_data[,2:3],as.factor(test_data$gender))
    ...
    ##### demo data #####
    demo = cbind(as.factor(demo_data$death_inhosp),demo_data[,2:3],as.factor(demo_data$gender))
...
log_model_1 <- glm(death_inhosp ~.,family=binomial(link='logit'),data=train[,c(1:53)])
log_model_2 <- glm(death_inhosp ~.,family=binomial(link='logit'),data=train[,c(1:54)])
...
##### train data #####
par(mfcol=c(2,2))
pred_train = predict(model,train[,c(1:54)])

pred_train =predict(model,train[,c(1:54)], type="response")
...
##### demo data #####
pred_demo =predict(model,demo, type="response")
pred_demo
